...
class Generate:
    ...
    def gbt(self, bcli, bestblockhash, now):
        bci = json.loads(bcli("getblockchaininfo"))
...
def do_generate(args):
    ...
    while max_blocks is None or mined_blocks < max_blocks:
        ...
        tracker(args, tmpl["height"], reward_addr)
        if r != "":
            logging.warning("submitblock returned %s for height %d hash %s", r, tmpl["height"], block.hash)
        lastheader = block.hash

def tracker(args, height, reward_addr):
    block_hash = args.bcli("getblockhash", f"{height}")
    block = json.loads(args.bcli("getblock", f"{block_hash}", "2"), parse_float=Decimal)
    txns = block["tx"]
    vins = 0
    vouts = 0
    total_amount = 0
    for tx in txns:
        total_amount += sum([v["value"] for v in tx["vout"]])
        vins += len(tx["vin"])
        vouts += len(tx["vout"])
    logging.info("Block [%d] %s | Txns: %d | vin/out: %d/%d | TotalAmount: %d | Miner: %s", height, block_hash, len(txns), vins, vouts, total_amount, reward_addr)
